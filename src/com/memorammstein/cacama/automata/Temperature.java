	...
	public void feed(String wrapperSymbol) {
		...
		case q0:
			if (temp > range.get("maximum")) {
				symbol = Temperature_Automaton.Alphabet.moreThanMaximum;
			} else if (temp < range.get("minimum")) {
				symbol = Temperature_Automaton.Alphabet.lessThanMinimum;
			}
			break;
		case q1:
			if (temp > range.get("maximum")) {
				...
				symbol = Temperature_Automaton.Alphabet.moreThanMinimum;
			}
			break;
		case q2:
			if (temp < range.get("maximum")) {
				symbol = Temperature_Automaton.Alphabet.lessThanMaximum;
			} else if (temp < range.get("minimum")) {
				symbol = Temperature_Automaton.Alphabet.lessThanMinimum;
			}
			break;
